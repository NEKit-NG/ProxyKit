...
open class IPPacket {
     ...
     */
    public static func peekSourcePort(_ data: Data) -> IPort? {
        ...
        return IPort(bytesInNetworkOrder: (data as NSData).bytes.advanced(by: headerLength))
    }
    ...
    /**
     ...
     - note: Only TCP and UDP packet has port field.
     */
    public static func peekDestinationPort(_ data: Data) -> IPort? {
        guard let proto = peekProtocol(data) else {
            return nil
        }
        ...
        return IPort(bytesInNetworkOrder: (data as NSData).bytes.advanced(by: headerLength + 2))
    }
