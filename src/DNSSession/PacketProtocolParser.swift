...
class UDPProtocolParser: TransportProtocolParserProtocol {
    /// The source port.
    var sourcePort: IPort!

    /// The destination port.
    var destinationPort: IPort!
    ...
    init?(packetData: Data, offset: Int) {
        ...
        sourcePort = IPort(bytesInNetworkOrder: (packetData as NSData).bytes.advanced(by: offset))
        destinationPort = IPort(bytesInNetworkOrder: (packetData as NSData).bytes.advanced(by: offset + 2))
